...
class _ZoomScaffoldState extends State<ZoomScaffold>
  ...
  bool get hasStartMenu => widget.menuScreen != null;
  bool get hasEndMenu => widget.endMenuScreen != null;
  ...
  Widget _buildGestures(bool dragFromLeft) {
    return GestureDetector(
      ...
      onHorizontalDragUpdate: (details) async {
        if (!provider.isOpen) {
          if (dragFromLeft) {
            ...
            if (hasEndMenu) {
              List<int> ints = [];
              for (var i = MediaQuery.of(context).size.width.round();
                  i > 1;
                  i--) ints.add(i);
      ...
      onHorizontalDragEnd: (detail) {
        ...
        if (dragFromLeft && provider.isStartMenu && !provider.isOpen && hasStartMenu)
          provider.open();
        if (!dragFromLeft && provider.isEndMenu && !provider.isOpen && hasEndMenu)
          provider.openEnd();
